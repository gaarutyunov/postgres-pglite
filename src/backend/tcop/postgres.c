...
/*
 * Report idle state to the frontend, if we have reached it.
 *
 * Lifted verbatim out of PostgresMain's message loop so the PGlite Emscripten
 * driver can call it directly: the pumped loop returns to JS between messages,
 * so the step that says "ready for the next query" has to be callable on its
 * own.  For a normal build it runs exactly where it always did.
 */
void
...
/*
 * Run one iteration of PostgresMain's message loop.
 *
 * Also lifted verbatim, for the same reason: a browser cannot block the main
 * thread, so PGlite drives the backend one message at a time rather than
 * letting it sit in ReadCommand.  Returning ends one iteration -- what used
 * to be `continue` is now `return`.
 */
void
PostgresMainLoopOnce(void)
{
	...
	/*
	 * (1) If we've reached idle state, tell the frontend we're ready for
	 * a new query.  The body lives in a top-level function so the PGlite
	 * driver can reach idle state on its own between pumped messages.
	 */
...
/*
 * Error recovery for PostgresMain: what the outermost sigsetjmp lands on.
 *
 * Top level so PGlite can longjmp here from a JS-driven message without
 * PostgresMain's frame being live.  Contents are PG19's verbatim.
 */
void
...
PostgresMain(const char *dbname, const char *username)
{
	 ...
	 * local_sigjmp_buf, which is never sigsetjmp'd.  PGlite never builds
	 * vanilla so it does not bite them, but here the first ereport(ERROR)
	 * would longjmp into an uninitialised jmp_buf.  A normal build keeps
